    ...
    <div className="relative rounded-2xl overflow-hidden mb-10">
      ...
      <div className="relative z-10 px-6 py-16 md:py-20 text-center">
        ...
        <motion.p initial={{ opacity:0,y:20 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.6,delay:0.1 }}
          className="text-base md:text-lg mb-8" style={{ color:"rgba(255,249,242,0.88)" }}>
          {t.home_subtitle}
        </motion.p>
        <motion.div initial={{ opacity:0,y:20 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.6,delay:0.2 }}
          className="max-w-5xl mx-auto bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-2xl">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
            <AirportSelect placeholder={t.home_origin}      value={origen}  onChange={setOrigen}  aeropuertos={aeropuertos} disabled={cargandoAeropuertos}/>
            <div className="flex justify-center">
              <button onClick={swap} className="p-2 rounded-full border-none cursor-pointer transition-all"
                style={{ background:"rgba(150,95,33,0.1)", transform:swapping?"rotate(180deg)":"rotate(0deg)" }}>
                <SwapIcon/>
              </button>
            </div>
            <AirportSelect placeholder={t.home_destination} value={destino} onChange={setDestino} aeropuertos={aeropuertos} disabled={cargandoAeropuertos}/>
            <div className="relative flex items-center">
              <span className="absolute left-3 z-10 pointer-events-none" style={{ color:"rgba(150,95,33,0.7)" }}><CalendarIcon/></span>
              <input type="date" value={fecha} min={today} onChange={e=>setFecha(e.target.value)}
                className="w-full pl-9 pr-3 py-3 text-sm rounded-lg outline-none border bg-[#faf7f2] dark:bg-gray-700 text-[#1a1208] dark:text-gray-100 border-[rgba(150,95,33,0.2)] dark:border-gray-600"/>
            </div>
          </div>
