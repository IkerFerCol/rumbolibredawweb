...
const Spinner = ({ dark = false }) => (
  <svg className="animate-spin w-5 h-5" viewBox="0 0 24 24" fill="none"
    style={{ color: dark ? "rgba(150,95,33,1)" : "rgba(255,249,242,1)" }}>
...
function Input({ value, onChange, placeholder, type = "text", maxLength, ...rest }) {
  return (
    <input type={type} value={value} onChange={onChange}
      ...
      className="h-10 px-3 rounded-xl text-[13px] w-full transition-all"
      style={inputStyle}
...
function Select({ value, onChange, children }) {
  return (
    <select value={value} onChange={onChange}
      className="h-10 px-3 rounded-xl text-[13px] w-full cursor-pointer transition-all"
      style={inputStyle}
...
const AirportSelect = ({ placeholder, value, onChange, aeropuertos, disabled }) => (
  <div style={{ position: "relative", width: "100%" }}>
    <span className="absolute left-3 top-1/2 -translate-y-1/2 z-10 pointer-events-none text-[rgba(150,95,33,0.7)]">
      <LocationIcon />
    </span>
    <select value={value} onChange={e => onChange(e.target.value)} disabled={disabled}
      className="w-full pl-9 pr-3 py-3 text-sm rounded-lg outline-none border border-[rgba(150,95,33,0.2)] bg-[#faf7f2] text-[#1a1208] transition-colors duration-200 cursor-pointer appearance-none dark:bg-gray-700 dark:text-gray-100 dark:border-gray-600">
      <option value="">{placeholder}</option>
      {aeropuertos.map(a => (
        <option key={a.id} value={a.codigoIata}>{a.codigoIata} – {a.ciudad}</option>
      ))}
    </select>
  </div>
);
...
const VueloCard = ({ vuelo, onSelect, aeropuertoMap }) => {
  ...
  const dur = (s, l) => {
    if (!s || !l) return "N/A";
    const d = new Date(l) - new Date(s);
    return `${Math.floor(d / 3600000)}h ${Math.floor((d % 3600000) / 60000)}min`;
  };

  const resolve = key => {
    const num = Number(key);
    return aeropuertoMap[num] || Object.values(aeropuertoMap).find(a => a.codigoIata === key);
  };

  const orig = resolve(vuelo.origenId || vuelo.origen);
  const dest = resolve(vuelo.destinoId || vuelo.destino);
  const origenCiudad = orig?.ciudad || vuelo.origen || "Origen";
  const destinoCiudad = dest?.ciudad || vuelo.destino || "Destino";
    ...
    <motion.div whileHover={{ y: -6 }}
      ...
      <div className="p-5">
        ...
        <div className="flex justify-between items-start mb-4">
          <div>
            <p className="font-bold text-[15px] text-[#1a1208] dark:text-gray-100">
              {vuelo.aerolineaNombre || "Aerolínea"}
            </p>
            <p className="text-[11px] text-[#9c8060] dark:text-gray-400">{vuelo.numeroVuelo}</p>
          </div>
          <div className="text-right">
            <p className="text-[20px] font-bold text-[#965f21]">€{vuelo.precio?.toFixed(2) || "0"}</p>
            <p className="text-[11px] text-[#9c8060]">por persona</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 mb-3">
          <div className="text-center flex-1">
            ...
            <p className="text-[11px] text-[#9c8060]">{origenCodigo}</p>
          </div>
          <div className="flex-1 px-1 text-center">
            <div className="relative flex items-center justify-center mb-1">
              ...
              <span className="relative bg-white dark:bg-gray-800 px-1">
                <PlaneIcon className="w-3.5 h-3.5" style={{ color: "rgba(150,95,33,0.5)" }} />
              </span>
            </div>
          ...
          <div className="text-center flex-1">
            ...
            <p className="text-[11px] text-[#9c8060]">{destinoCodigo}</p>
          </div>
        </div>

        <p className="text-[12px] text-center text-[#9c8060] mb-1">
...
const HeroSection = ({ onSearch, aeropuertos, cargandoAeropuertos }) => {
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden mb-10">
      ...
      <div className="absolute inset-0" style={{ background: "linear-gradient(135deg,rgba(0,0,0,0.72) 0%,rgba(0,0,0,0.48) 100%)" }} />
      <div className="relative z-10 px-6 py-16 md:py-20 text-center">
        ...
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}
          className="text-base md:text-lg mb-8 text-[rgba(255,249,242,0.88)]">
          Los mejores precios en vuelos con RumboLibre
        </motion.p>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
          className="max-w-5xl mx-auto bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-2xl">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
            <AirportSelect placeholder="Origen" value={origen} onChange={setOrigen} aeropuertos={aeropuertos} disabled={cargandoAeropuertos} />
            <div className="flex justify-center">
              <button onClick={swap} title="Intercambiar"
                className="p-2 rounded-full border-none cursor-pointer transition-all"
                style={{ background: "rgba(150,95,33,0.1)", transform: swapping ? "rotate(180deg)" : "rotate(0deg)" }}>
                <SwapIcon />
              </button>
            </div>
            <AirportSelect placeholder="Destino" value={destino} onChange={setDestino} aeropuertos={aeropuertos} disabled={cargandoAeropuertos} />
            <div className="relative flex items-center">
              <span className="absolute left-3 z-10 pointer-events-none text-[rgba(150,95,33,0.7)]"><CalendarIcon /></span>
              <input type="date" value={fecha} min={today} onChange={e => setFecha(e.target.value)}
                className="w-full pl-9 pr-3 py-3 text-sm rounded-lg outline-none border border-[rgba(150,95,33,0.2)] bg-[#faf7f2] text-[#1a1208] dark:bg-gray-700 dark:text-gray-100 dark:border-gray-600" />
            </div>
          </div>
          <button onClick={() => onSearch({ origen, destino, fecha })}
            className="w-full mt-3 py-3 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 border-none cursor-pointer text-white transition-all"
            style={{ background: "rgba(150,95,33,1)" }}
            onMouseEnter={e => (e.currentTarget.style.background = "rgba(110,68,18,1)")}
            onMouseLeave={e => (e.currentTarget.style.background = "rgba(150,95,33,1)")}>
...
// ─── Modal de reserva (drawer) ────────────────────────────────────────────────
function ModalReserva({ vuelo, aeropuertoMap, user, onClose, onSuccess }) {
  const navigate = useNavigate();

  // Estado formulario
  const [paso, setPaso] = useState(1); // 1=pasajeros, 2=pago
  ...
  function pasajeroVacio(u) {
    return {
      ...
      apellidos: u?.name?.split(" ").slice(1).join(" ") || "",
      tipoDocumento: "DNI", numeroDocumento: "", fechaNacimiento: "", nacionalidad: "",
    };
  }

  function ajustarPax(n) {
    const num = Math.max(1, Math.min(9, n));
    setNumPax(num);
    setPasajeros(prev => {
      if (num > prev.length)
        return [...prev, ...Array.from({ length: num - prev.length }, () => pasajeroVacio(null))];
      return prev.slice(0, num);
    });
  }

  function updatePax(i, field, val) {
    setPasajeros(prev => { const c = [...prev]; c[i] = { ...c[i], [field]: val }; return c; });
  }

  function updatePago(field, val) {
    setPago(p => ({ ...p, [field]: val }));
  }
  ...
  function validarPaso1() {
    for (let i = 0; i < pasajeros.length; i++) {
      const p = pasajeros[i];
      if (!p.nombre.trim()) return `Pasajero ${i + 1}: falta el nombre`;
      if (!p.apellidos.trim()) return `Pasajero ${i + 1}: faltan los apellidos`;
      if (!p.numeroDocumento.trim()) return `Pasajero ${i + 1}: falta el número de documento`;
    }
    return null;
  }
  ...
  function validarPaso2() {
    if (pago.metodoPago !== "PAYPAL") {
      if (
        !pago.numeroTarjeta ||
        pago.numeroTarjeta.replace(/\s/g, "").length < 16
      )
        return "Número de tarjeta inválido";
      if (!pago.titular.trim()) return "Falta el titular";
      if (!pago.caducidad || pago.caducidad.length < 5)
        return "Caducidad inválida";
      if (!pago.cvv || pago.cvv.length < 3) return "CVV inválido";
    }
    // Si es PayPal, no requiere validación adicional de tarjeta
    return null;
  }
  ...
  // Resolver nombres de ciudad/código
  const resolve = key => {
    const num = Number(key);
    return aeropuertoMap[num] || Object.values(aeropuertoMap).find(a => a.codigoIata === key);
  };
  ...
  const origenCodigo = orig?.codigoIata || vuelo.origen || "—";
  const destinoCodigo = dest?.codigoIata || vuelo.destino || "—";
  const origenCiudad = orig?.ciudad || "";
  const destinoCiudad = dest?.ciudad || "";
  ...
  async function confirmar() {
    ...
    try {
      ...
      // 2. Procesar pago - Construir payload condicionalmente
      const payloadPago = {
        reservaId,
        metodoPago: pago.metodoPago,
      };

      // Solo incluir datos de tarjeta si NO es PayPal
      if (pago.metodoPago !== "PAYPAL") {
        payloadPago.numeroTarjeta = pago.numeroTarjeta?.replace(/\s/g, "") || null;
        payloadPago.titular = pago.titular || null;
        payloadPago.caducidad = pago.caducidad || null;
        payloadPago.cvv = pago.cvv || null;
      }
    ...
    <AnimatePresence>
      ...
      <motion.div
        ...
        style={{
          ...
          borderLeft: "0.5px solid rgba(150,95,33,0.18)",
          boxShadow: "-8px 0 40px rgba(150,95,33,0.15)",
          fontFamily: "'DM Sans', sans-serif",
        }}
      ...
      >
        ...
        <div className="px-6 py-4 flex items-center justify-between shrink-0"
          style={{
            background: "linear-gradient(135deg,rgba(150,95,33,0.12) 0%,rgba(200,160,100,0.06) 100%)",
            borderBottom: "0.5px solid rgba(150,95,33,0.14)",
          }}>
          <div>
            <h2 className="text-[17px] font-semibold" style={{ fontFamily: "'Playfair Display', serif", color: "rgba(26,18,8,1)" }}>
              Completar reserva
            </h2>
            <p className="text-[12px]" style={{ color: "rgba(156,128,96,1)" }}>
              {origenCodigo} → {destinoCodigo} · €{total.toFixed(2)}
            </p>
          </div>
          <button onClick={onClose}
            className="w-8 h-8 rounded-full flex items-center justify-center text-[20px] transition-all border-none cursor-pointer"
        ...
        <div className="px-6 py-4 shrink-0" style={{ borderBottom: "0.5px solid rgba(150,95,33,0.1)" }}>
          <div className="flex items-center justify-between">
            <div className="text-center">
              ...
              <p className="text-[11px]" style={{ color: "rgba(156,128,96,1)" }}>{origenCiudad}</p>
            ...
            <div className="text-center">
              ...
              <p className="text-[11px]" style={{ color: "rgba(156,128,96,1)" }}>{destinoCiudad}</p>
        ...
        <div className="px-6 py-3 flex items-center gap-3 shrink-0" style={{ borderBottom: "0.5px solid rgba(150,95,33,0.08)" }}>
          {[{ n: 1, label: "Pasajeros" }, { n: 2, label: "Pago" }].map(({ n, label }) => (
            <React.Fragment key={n}>
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold transition-all"
                  style={{
                    background: paso >= n ? "rgba(150,95,33,1)" : "rgba(150,95,33,0.12)",
                    color: paso >= n ? "rgba(255,249,242,1)" : "rgba(150,95,33,0.5)",
                  }}>
                  {n}
                </div>
                <span className="text-[12px] font-medium" style={{ color: paso >= n ? "rgba(26,18,8,1)" : "rgba(156,128,96,1)" }}>
                  {label}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between mb-5 pb-4"
                ...
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => ajustarPax(numPax - 1)} disabled={numPax <= 1}
                    ...
                    style={{ background: numPax <= 1 ? "rgba(150,95,33,0.08)" : "rgba(150,95,33,1)", color: numPax <= 1 ? "rgba(150,95,33,0.3)" : "rgba(255,249,242,1)" }}>
                    −
                  </button>
                  <span className="text-[18px] font-bold w-6 text-center" style={{ color: "rgba(150,95,33,1)" }}>
                    {numPax}
                  </span>
                  <button type="button" onClick={() => ajustarPax(numPax + 1)} disabled={numPax >= 9}
                    className="w-8 h-8 rounded-full flex items-center justify-center text-lg font-bold border-none cursor-pointer transition-all"
                    style={{ background: numPax >= 9 ? "rgba(150,95,33,0.08)" : "rgba(150,95,33,1)", color: numPax >= 9 ? "rgba(150,95,33,0.3)" : "rgba(255,249,242,1)" }}>
                    +
                  </button>
                </div>
              </div>

              {/* Formulario cada pasajero */}
              {pasajeros.map((p, i) => (
                <div key={i} className="mb-6">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <p className="text-[13px] font-semibold" style={{ color: "rgba(26,18,8,1)" }}>
                      Pasajero {i + 1}
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    <Campo label="Nombre" required>
                      <Input value={p.nombre} onChange={e => updatePax(i, "nombre", e.target.value)} placeholder="Ana" />
                    </Campo>
                    <Campo label="Apellidos" required>
                      <Input value={p.apellidos} onChange={e => updatePax(i, "apellidos", e.target.value)} placeholder="García" />
                    </Campo>
                    <Campo label="Tipo doc." required>
                      <Select value={p.tipoDocumento} onChange={e => updatePax(i, "tipoDocumento", e.target.value)}>
                        <option value="DNI">DNI</option>
                        <option value="PASAPORTE">Pasaporte</option>
                        <option value="NIE">NIE</option>
                      </Select>
                    </Campo>
                    <Campo label="Nº documento" required>
                      <Input value={p.numeroDocumento} onChange={e => updatePax(i, "numeroDocumento", e.target.value)} placeholder="12345678A" />
                    </Campo>
                    <Campo label="Fecha nacimiento">
                      <Input type="date" value={p.fechaNacimiento} onChange={e => updatePax(i, "fechaNacimiento", e.target.value)} />
                    </Campo>
                    <Campo label="Nacionalidad">
                      <Input value={p.nacionalidad} onChange={e => updatePax(i, "nacionalidad", e.target.value)} placeholder="Española" />
                    </Campo>
                  </div>

                  {i < pasajeros.length - 1 && (
                    <div className="mt-4" style={{ borderBottom: "0.5px solid rgba(150,95,33,0.1)" }} />
                  )}
                </div>
              ))}
            </div>
          ...
          {paso === 2 && (
            <div>
              ...
              <div className="rounded-xl p-4 mb-5"
                style={{ background: "rgba(150,95,33,0.06)", border: "0.5px solid rgba(150,95,33,0.15)" }}>
                {[
                  { label: `Vuelo × ${numPax}`, val: `€${(vuelo.precio || 0).toFixed(2)} × ${numPax}` },
                  { label: "Tasas incluidas", val: "✓" },
                  { label: "Equipaje de mano", val: "✓" },
                ].map(row => (
                  <div key={row.label} className="flex justify-between py-1.5 text-[12px]"
                    style={{ borderBottom: "0.5px solid rgba(150,95,33,0.08)" }}>
                    <span style={{ color: "rgba(156,128,96,1)" }}>{row.label}</span>
                    <span style={{ color: "rgba(92,74,42,1)", fontWeight: 500 }}>{row.val}</span>
                  </div>
                ))}
                <div className="flex justify-between items-center mt-2 pt-1">
                  <span className="text-[14px] font-bold" style={{ color: "rgba(26,18,8,1)" }}>Total</span>
                  <span className="text-[20px] font-bold" style={{ color: "rgba(150,95,33,1)" }}>€{total.toFixed(2)}</span>
                </div>
              </div>

              {/* Método de pago */}
              <p className="text-[11px] uppercase tracking-widest mb-3" style={{ color: "rgba(92,74,42,1)" }}>
                Método de pago
              </p>
              ...
              {(pago.metodoPago === "TARJETA_CREDITO" || pago.metodoPago === "TARJETA_DEBITO") && (
                <div className="flex flex-col gap-3">
                  <Campo label="Número de tarjeta" required>
                    <Input value={pago.numeroTarjeta}
                      onChange={e => updatePago("numeroTarjeta", fmtCard(e.target.value))}
                      placeholder="1234 5678 9012 3456" maxLength={19} />
                  </Campo>
                  <Campo label="Titular" required>
                    <Input value={pago.titular}
                      onChange={e => updatePago("titular", e.target.value.toUpperCase())}
                      placeholder="ANA GARCÍA LÓPEZ" />
                  </Campo>
                  <div className="grid grid-cols-2 gap-3">
                    <Campo label="Caducidad" required>
                      <Input value={pago.caducidad}
                        onChange={e => updatePago("caducidad", fmtExpiry(e.target.value))}
                        placeholder="MM/AA" maxLength={5} />
                    </Campo>
                    <Campo label="CVV" required>
                      <Input type="password" value={pago.cvv}
                        onChange={e => updatePago("cvv", e.target.value.replace(/\D/g, "").slice(0, 4))}
              ...
              {pago.metodoPago === "PAYPAL" && (
                <div className="rounded-xl p-4 text-center"
                  style={{ background: "rgba(150,95,33,0.05)", border: "0.5px solid rgba(150,95,33,0.15)" }}>
        ...
        <div className="px-6 py-4 shrink-0"
          style={{ borderTop: "0.5px solid rgba(150,95,33,0.12)", background: "rgba(255,249,242,1)" }}>

          {/* Error */}
          {error && (
            <div className="rounded-xl px-3 py-2 mb-3 text-[12px]"
              style={{ background: "rgba(180,50,50,0.08)", border: "0.5px solid rgba(180,50,50,0.25)", color: "rgba(180,50,50,1)" }}>
              ⚠️ {error}
            </div>
          )}
          ...
          <div className="flex gap-3">
            ...
            {paso === 1 ? (
              ...
              <button onClick={confirmar} disabled={loading}
                ...
                {loading ? <><Spinner /> Procesando...</> : `✈ Pagar €${total.toFixed(2)}`}
              </button>
            )}
          </div>

          <p className="text-[10px] text-center mt-2" style={{ color: "rgba(156,128,96,1)" }}>
      ...
      <motion.div initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <div className="px-8 py-8 text-center"
          ...
          <p className="text-[13px]" style={{ color: "rgba(156,128,96,1)" }}>
            Tu vuelo ha sido reservado con éxito
          </p>
        </div>

        <div className="px-8 py-6">
          {[
            { label: "Código", val: reserva?.codigoReserva || "—", hl: true },
            { label: "Estado", val: "Confirmada ✅" },
            { label: "Total pag.", val: `€${Number(reserva?.precioTotal || 0).toFixed(2)}` },
          ].map(r => (
            <div key={r.label} className="flex justify-between py-2.5"
              ...
              <span className="text-[13px] font-bold"
                style={{ color: r.hl ? "rgba(150,95,33,1)" : "rgba(26,18,8,1)" }}>{r.val}</span>
            </div>
          ))}
...
export default function HomePage() {
  ...
  const aeropuertoMap = useMemo(() => {
    const m = {};
    aeropuertos.forEach(a => { m[a.id] = a; });
    return m;
  }, [aeropuertos]);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (r.data?.length) setAeropuertos(r.data);
      } catch { /* usa estáticos */ }
  ...
  const buscarVuelos = useCallback(async ({ origen, destino, fecha }) => {
    ...
    try {
      ...
      setResultados(data);
      if (!data.length) setError("No hay vuelos disponibles para esa búsqueda.");
    } catch {
  ...
  function onReservaExitosa(reserva) {
      ...
      <div className="min-h-screen flex flex-col bg-[#fff9f2] dark:bg-gray-900 transition-colors duration-300"
            ...
            <AnimatePresence>
              {buscando && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  <p className="mt-3 text-[#9c8060]">Buscando vuelos…</p>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            {!buscando && resultados && (
              <>
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-2xl font-bold text-[#1a1208] dark:text-gray-100"
                    style={{ fontFamily: "'Playfair Display', serif" }}>
                    Resultados de búsqueda
                    <span className="text-[14px] font-normal ml-2 text-[#9c8060]">
                      ({resultados.length} vuelo{resultados.length !== 1 ? "s" : ""})
                    </span>
                  </h2>
                  <button onClick={() => { setResultados(null); setError(""); }}
                    className="text-sm font-medium text-[#965f21] hover:opacity-70 transition-opacity bg-transparent border-none cursor-pointer">
                    ← Mostrar todos
                  </button>
                </div>
                ...
                {resultados.length === 0 ? (
                  <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-xl">
                    ...
                    <p className="text-[#9c8060]">No hay vuelos disponibles.</p>
                  </div>
                ) : (
        ...
        {vueloSeleccionado && (
          <ModalReserva
            ...
            aeropuertoMap={aeropuertoMap}
            user={user}
            onClose={() => setVueloSeleccionado(null)}
            onSuccess={onReservaExitosa}
          />
        )}
        ...
        {reservaExitosa && (
          <ModalExito
            ...
            onClose={() => setReservaExitosa(null)}
          />
        )}
      </div>
    </>
