    ...
    <div className="rounded-2xl overflow-hidden transition-all duration-300 bg-white dark:bg-gray-800"
      ...
      <div className="p-6">
        ...
        <div className="flex items-center gap-3 pt-3" style={{ borderTop:"0.5px solid rgba(150,95,33,0.1)" }}>
          <button onClick={()=>onLike(tema.id)}
            className="flex items-center gap-1.5 text-[13px] font-medium px-3 py-1.5 rounded-lg transition-all text-[#9c8060] hover:bg-[rgba(150,95,33,0.07)] border-none cursor-pointer bg-transparent">
            🤍 {tema.likes||0} {t.foro_like}
          </button>
...
export default function ForoPage() {
  ...
  async function handleLike(id){
    ...
    try{ await api.post(`/foro/likes/tema/${id}?usuarioId=${user.id}`); setTemas(prev=>prev.map(t=>t.id===id?{...t,likes:(t.likes||0)+1}:t)); }
    catch{cargarTemas();}
  }
